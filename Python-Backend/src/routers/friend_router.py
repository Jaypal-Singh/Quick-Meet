...
async def add_friend(data: AddFriendRequest, token: Optional[str] = Cookie(None)):
    if not data.token:
        data.token = token
    return await friend_controller.add_friend(data)

@router.post("/accept")
async def accept_friend(data: AddFriendRequest, token: Optional[str] = Cookie(None)):
    if not data.token:
        data.token = token
    return await friend_controller.accept_friend_request(data)

@router.post("/reject")
async def reject_friend(data: AddFriendRequest, token: Optional[str] = Cookie(None)):
    if not data.token:
        data.token = token
    return await friend_controller.reject_friend_request(data)

@router.post("/invite")
async def invite_friend(data: InviteRequest, token: Optional[str] = Cookie(None)):
    if not data.token:
        data.token = token
