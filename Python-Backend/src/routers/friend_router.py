...
async def add_friend(data: AddFriendRequest, token: str = Cookie(None)):
    data.token = token
    return await friend_controller.add_friend(data)

@router.post("/accept")
async def accept_friend(data: AddFriendRequest, token: str = Cookie(None)):
    data.token = token
    return await friend_controller.accept_friend_request(data)

@router.post("/reject")
async def reject_friend(data: AddFriendRequest, token: str = Cookie(None)):
    data.token = token
    return await friend_controller.reject_friend_request(data)

@router.post("/invite")
async def invite_friend(data: InviteRequest, token: str = Cookie(None)):
    data.token = token
    return await friend_controller.invite_to_meeting(data)

@router.get("/list")
async def get_friends(token: str = Cookie(None)):
